...
def filter_types(record: SlaughterRecord) -> bool:
  return record.purchase_type in \
    ('Prod. Sold Negotiated', 'Prod. Sold Swine or Pork Market Formula', 'Prod. Sold Negotiated Formula')
...
def compute_cash_index(records: Iterator[SlaughterRecord]) -> DataFrame:
  data = DataFrame.from_records(records, columns=SlaughterRecord._fields)

  data['purchase_type'] = data['purchase_type'].replace({
  ...
  data = data.set_index(['date', 'purchase_type'])

  total_weight = compute_weight(data['head_count'], data['carcass_weight'])
  total_value = compute_value(total_weight, data['net_price'])
  ...
  cash_index = compute_price(rolling_totals['total_value'], rolling_totals['total_weight']).rename('cme_index')

  table = pd.concat([data['head_count'], data['carcass_weight'], price], axis=1).unstack()
  ...
  index_change = (cash_index - cash_index.shift(1)).rename('index_change')

  return pd.concat([table, daily_price, price_change, cash_index, index_change], axis=1)
