...
attributes = list(load_resource('test/api/resources/slaughter.xml'))
assert len(attributes) == 8

negotiated = Slaughter.from_attributes(attributes[0])
negotiated_formula = Slaughter.from_attributes(attributes[4])
...
class NegotiatedPurchaseTest(TestCase):
    ...
    def test_seller(self):
        self.assertEqual(negotiated.seller, Seller.PRODUCER.to_ordinal())

    def test_arrangement(self):
        self.assertEqual(negotiated.arrangement, Arrangement.NEGOTIATED.to_ordinal())

    def test_basis(self):
        self.assertEqual(negotiated.basis, Basis.ALL.to_ordinal())
